...
**Illustrative Scenario: The Bottling Plant Transformation**
Consider a typical bottling plant scenario that illustrates the limitations of Approach A. With vibration sensors installed on fillers, alerts become unreliable because they trigger every time the filler changes bottle sizes (a normal process variation). The solution is shifting to Approach B: installing a DataOps gateway that pulls the "Recipe ID" from the PLC and combines it with vibration data in the Unified Namespace. The analytics model can then use dynamic thresholds: "If Recipe = 500ml, Limit = 0.5 in/s. If Recipe = 2L, Limit = 0.8 in/s." Organizations implementing this approach typically report significant reductions in false positives. This is particularly critical for a [high-speed beverage line](/blog/why-your-high-speed-beverage-line-is-stuck-at-75-oee-and-how-a-maintenance-first-strategy-unlocks-the-rest) where small inefficiencies compound quickly. This is the power of the UNS Overlay—it allows you to inject logic into the data stream before it reaches the dashboard.
