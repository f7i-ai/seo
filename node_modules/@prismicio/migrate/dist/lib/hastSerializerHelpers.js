import { OEmbedType, LinkType } from "@prismicio/client";
...
const serializeImage = (node) => {
  ...
  if (!src) {
    throw new PrismicRichTextSerializerError("Element of type `img` is missing an `src` attribute");
  }
...
const serializeEmbed = (node) => {
  ...
  const src = (_a = node.properties) == null ? void 0 : _a.src;
  if (!src) {
    throw new PrismicRichTextSerializerError("Element of type `embed` is missing an `src` attribute");
  }
  const oembedBase = {
    version: "1.0",
    embed_url: src,
...
const serializeSpan = (node, rtPartialInlineNode) => {
  ...
  if (rtPartialInlineNode.type === "hyperlink" && !("data" in rtPartialInlineNode)) {
    ...
    if (!url) {
      throw new PrismicRichTextSerializerError("Element of type `hyperlink` is missing an `href` attribute");
    }
    return {
      type: "hyperlink",
      data: {
...
//# sourceMappingURL=hastSerializerHelpers.js.map
